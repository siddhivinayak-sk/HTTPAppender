 ...
 *
 */
@Plugin(name="HttpAppender", category="Core", elementType="appender", printObject=true)
public class HttpAppender extends AbstractAppender {
	...
	protected HttpAppender(String name, Filter filter, Layout<? extends Serializable> layout) {
    ...
    public static HttpAppender createAppender(
		...
		HttpAppender temp = new HttpAppender(name, filter, layout, true);
		temp.setLogURL(host);
        return temp;
    }	
}
